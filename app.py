...
# ==========================================
# GIAO DIỆN CSS (Sửa lỗi mất tính năng & Căn chỉnh logo)
# ==========================================
st.markdown("""
<style>
    ...
    /* Thu nhỏ logo và bo góc chỉn chu */
...
# ==========================================
# 1. HỆ THỐNG ĐĂNG NHẬP (PHỤC HỒI ĐẦY ĐỦ)
# ==========================================
if not st.session_state['logged_in']:
    col1, col2, col3 = st.columns([1, 1.8, 1])
    with col2:
        st.markdown('<div class="auth-box">', unsafe_allow_html=True)
        # Hiển thị Logo thu nhỏ ở trang login
        ...
        with tab_reg:
            ...
            if st.button("ĐĂNG KÝ NGAY", use_container_width=True):
                ...
                elif len(reg_pwd) < 6: st.error("Mật khẩu tối thiểu 6 ký tự!")
                else:
                    save_user_api(reg_user, hash_password(reg_pwd), reg_email)
                    st.success("✅ Đăng ký thành công! Mời bạn quay lại Đăng nhập.")
        ...
        with tab_forgot:
            if not st.session_state['otp_verified']:
                ...
                if st.button("Gửi mã xác nhận", use_container_width=True):
                    if fg_user in users_db and users_db[fg_user].get("email") == fg_email:
                        ...
                        if reset_password_api(fg_user, fg_email, hash_password(otp), True):
                            if send_otp_email(fg_email, fg_user, otp): st.success("✅ Mã OTP đã gửi vào Email!")
                ...
                new_p = st.text_input("Mật khẩu mới hoàn toàn", type="password", key="new_p")
...
# ==========================================
# 2. DASHBOARD CHÍNH (VỚI HEADER MỚI)
# ==========================================
else:
    # Header chào mừng & Nút Đăng xuất ở góc
    ...
    # Logo thu nhỏ căn giữa chỉn chu
    ...
    with col_left:
        ...
        st.markdown("---")
        s_sign = st.text_area("🖋️ Chữ ký liên hệ cuối thư:", value=st.session_state.get('s_sign', "Trân trọng,\nĐội ngũ hỗ trợ Trường Sơn"), height=100)
    ...
    with col_right:
        ...
        raw_body = st.text_area("Nội dung chính (gõ xuống dòng tự do):", height=200, value="Chào {{name}},...")
        
        # Xử lý nội dung hiển thị chuẩn bài bản
    ...
    with st.expander("🔔 Nhận báo cáo kết quả qua Telegram"):
        ...
        if st.button("💾 Lưu cấu hình Telegram"):
            if save_config_api(st.session_state['current_user'], new_tk, new_id):
                st.success("✅ Đã lưu cấu hình báo cáo!"); time.sleep(1); st.rerun()
    ...
    if st.button("▶ BẮT ĐẦU CHIẾN DỊCH", type="primary", use_container_width=True):
        ...
        else: st.error("Vui lòng nhập đủ cấu hình gửi và danh sách!")
