...
# ==========================================
    ...
    with col_left:
        ...
        up = st.file_uploader("Danh sách Excel (email, name)", type=["csv", "xlsx"])
        df = None
        if up:
    ...
    with col_right:
        ...
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            {body_html}
            <br><br>
            <div style="color: #666; border-top: 1px solid #eee; padding-top: 10px;">
                {sign_html}
            </div>
        </div>
        """
    ...
    if st.button("▶ BẮT ĐẦU CHIẾN DỊCH", type="primary", use_container_width=True):
        if df is not None and s_mail and s_pass:
            ...
            log_expander = st.expander("📋 Nhật ký", expanded=True)
            ...
            # TỔNG KẾT & GỬI FILE TELE
            final_df = pd.DataFrame({"Email": success_list + [e.split()[0] for e in error_list], "Kết quả": ["Thành công"]*len(success_list) + ["Lỗi"]*len(error_list)})
            csv_buf = io.BytesIO()
            ...
            send_tele_file(t_tk, t_id, csv_buf.getvalue(), "bao_cao.csv")
            st.success("🎉 Hoàn tất!")
