...
# ==========================================
# 1. HỆ THỐNG ĐĂNG NHẬP (QUAY LẠI BẢN GỐC)
# ==========================================
...
# ==========================================
# 2. DASHBOARD CHÍNH (GIỮ LẠI ĐẦY ĐỦ CHỨC NĂNG GỬI)
# ==========================================
else:
    ...
    with col_left:
        ...
        with st.expander("❓ Cách lấy App Password nhanh nhất"):
            st.markdown("""
            <div class="help-box">
                <b>Bảo mật của Google yêu cầu dùng Mật khẩu ứng dụng:</b><br>
                1. Truy cập <a href="https://myaccount.google.com/security" target="_blank">Bảo mật Google</a>.<br>
                2. Đảm bảo đã bật <b>Xác minh 2 bước</b>.<br>
                3. Tìm mục <b>Mật khẩu ứng dụng</b>, tạo mã mới tên 'BulkMail' và copy 16 ký tự dán vào ô trên.
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_right:
        ...
        with st.expander("👁️ Xem trước thực tế", expanded=True):
            ...
            st.markdown(full_email_content.replace("{{name}}", f"<b>{ex_name}</b>"), unsafe_allow_html=True)
    ...
    st.warning("⚠️ **CẢNH BÁO AN TOÀN:** Mỗi tài khoản Gmail chỉ nên gửi an toàn khoảng **200 - 300 email/ngày**.")
    ...
    if st.button("▶ BẮT ĐẦU CHIẾN DỊCH", type="primary", use_container_width=True):
        if df is not None and s_mail and s_pass:
            ...
            if not e_col: st.error("❌ File thiếu cột Email!")
                ...
                clean_pass = s_pass.replace(" ", "").strip()
                ...
                for index, row in df.iterrows():
                    ...
                    try:
                        ...
                        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context, timeout=15) as server:
                            server.login(s_mail, clean_pass); server.send_message(msg)
