...
from email.mime.image import MIMEImage # <-- Thêm MIMEImage để nhúng ảnh an toàn
...
import base64
import os
import re # <-- Thêm re để xử lý mã Base64
from bs4 import BeautifulSoup # <-- Thêm để tự động tìm và nhúng ảnh dán vào
from streamlit_quill import st_quill # <-- Thêm Quill thay thế st.text_area
...
# ==========================================
    ...
    with col_content:
        ...
        # --- ĐÃ SỬA: Thay thế Text Area thành Quill Editor để có thể dán ảnh ---
        st.markdown("<p style='font-size: 14px; font-weight: 600; color: #334155; margin-bottom: 5px;'>Nội dung (Mẹo: Có thể dán trực tiếp hình ảnh vào đây):</p>", unsafe_allow_html=True)
        
        # Sử dụng trình soạn thảo Quill
        raw_body = st_quill(placeholder="Soạn nội dung hoặc dán ảnh vào đây...", html=True, key="quill_editor")
    ...
    with col_action2:
        ...
        if st.button("🚀 BẮT ĐẦU CHIẾN DỊCH GỬI MAIL", type="primary", use_container_width=True):
            if df is None:
                ...
                # --- ĐÃ SỬA: Xử lý đóng gói hình ảnh (CID) để Gmail hiển thị an toàn ---
                log.write("🔄 Đang xử lý nội dung và hình ảnh...")
                
                soup = BeautifulSoup(full_email_content, "html.parser")
                inline_images = []
                img_counter = 0
                
                # Quét tìm tất cả các thẻ ảnh trong văn bản
                for img in soup.find_all("img"):
                    src = img.get("src", "")
                    if not src: continue
                    
                    # Nếu ảnh dán từ trình soạn thảo (dạng mã Base64)
                    if src.startswith("data:image"):
                        try:
                            img_counter += 1
                            cid = f"img_inline_{img_counter}"
                            
                            # Tách phần mã và chuyển thành ảnh thật
                            header, encoded = src.split(",", 1)
                            img_data = base64.b64decode(encoded)
                            
                            # Lấy định dạng ảnh (png, jpeg)
                            ext = "png"
                            ...
                            inline_images.append({"cid": cid, "data": img_data, "type": ext})
                            
                            # Thay thế mã Base64 dài dòng bằng đường dẫn CID nội bộ
                            img["src"] = f"cid:{cid}"
                        except Exception:
                            pass

                prepared_html_template = str(soup) 
                log.write(f"✅ Đã mã hóa {len(inline_images)} hình ảnh vào thư.")
                # -----------------------------------------------------------------------
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # --- ĐÃ SỬA: Đóng gói email theo chuẩn Multipart/Mixed ---
                        ...
                        # Đính kèm ảnh đã xử lý vào khung thư
                        ...
                        # Đính kèm file đính kèm thông thường (Word, PDF) nếu có
                        if attachments:
