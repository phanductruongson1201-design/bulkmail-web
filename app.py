...
from email.mime.image import MIMEImage # Mới: Để xử lý ảnh nhúng
...
import base64
import os
import re # Mới: Để tìm mã ảnh trong nội dung
from bs4 import BeautifulSoup # Mới: Để xử lý nội dung HTML khi dán ảnh
from streamlit_quill import st_quill # Mới: Ô soạn thảo cho phép dán ảnh
...
# ==========================================
# GIAO DIỆN CSS (Giữ nguyên toàn bộ)
# ==========================================
st.markdown("""
<style>
...
if not st.session_state["logged_in"]:
    ...
    with col2:
        ...
        # (Giữ nguyên logic các tab Đăng ký và Quên MK của bạn)
...
# ==========================================
    ...
    with col_content:
        ...
        # SỬA: Thay đổi sang Quill Editor để bạn có thể DÁN HÌNH ẢNH
        raw_body = st_quill(placeholder="Dán chữ và ảnh vào đây...", html=True, key="quill_editor")
        ...
        if df is None or not subject or not st.session_state["s_email"]: st.error("⚠️ Thiếu thông tin gửi!")
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # SỬA: Xử lý kỹ thuật Nhúng ảnh (CID) khi gửi thư
                    ...
                    for img in soup.find_all("img"):
                        ...
                        if src.startswith("data:image"):
                            ...
                            img_part.add_header("Content-ID", f"<{cid}>"); msg.attach(img_part); img["src"] = f"cid:{cid}"
