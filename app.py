...
# ==========================================
    ...
    with col_content:
        ...
        # --- ĐÃ SỬA: Quill Editor dán thả ga ---
        st.markdown("<p style='font-size: 14px; font-weight: 600; color: #334155; margin-bottom: 5px;'>Nội dung (Bôi đen bài viết trên web rồi Paste thẳng vào đây):</p>", unsafe_allow_html=True)
        raw_body = st_quill(placeholder="Bôi đen copy văn bản và hình ảnh từ web, sau đó dán (Paste) vào đây...", html=True, key="quill_editor")
    ...
    with col_action2:
        ...
        if st.button("🚀 BẮT ĐẦU CHIẾN DỊCH GỬI MAIL", type="primary", use_container_width=True):
            if df is None:
                ...
                # --- ĐÃ SỬA: Robot tự phân tích, moi link ẩn và DỌN RÁC để vượt bảo mật Google ---
                log.write("🔄 Đang tối ưu hóa cấu trúc thư chống lỗi Google...")
                soup = BeautifulSoup(full_email_content, "html.parser")
                
                # XÓA TOÀN BỘ rác/script độc từ web
                ...
                for img in soup.find_all("img"):
                    # 1. Tìm đúng link ảnh thật (Ưu tiên data-src chống Lazy Load)
                    src = ""
                    for attr in ["data-src", "data-lazy-src", "data-original", "src"]:
                        val = img.get(attr)
                        # Vứt bỏ mấy cái ảnh placeholder "image.png" vô dụng gây lỗi hiển thị
                        if val and "image.png" not in val and "data:image/gif" not in val and "data:image/svg" not in val:
                            src = val
                            break
                            
                    if not src: 
                        img.decompose() # Xóa luôn khung ảnh lỗi để thư đẹp
                        continue
                    ...
                    # 3. Kỹ thuật chống lỗi bảo mật 5.7.0 và lỗi dung lượng 25MB
                    if src.startswith("http"):
                        # CHÌA KHÓA: Giữ nguyên link, để Gmail TỰ TẢI. Không đính kèm vào thư!
                        # Xóa sạch các thuộc tính rác của trang web đi
                        img.attrs = {"src": src, "style": "max-width: 100%; height: auto; display: block; margin: 10px 0;", "alt": "Hình ảnh"}
                        
                    elif src.startswith("data:image"):
                        # Nếu người dùng có dán thêm ảnh (Snipping Tool) thì mới nhúng CID
                        try:
                            img_counter += 1
                            ...
                            inline_images.append({
                                "cid": cid, "data": img_data, "type": ext, 
                ...
                log.write(f"✅ Tối ưu hoàn tất! Bức thư của bạn rất nhẹ và cực kỳ an toàn.")
                # ----------------------------------------------------------------------------------
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # --- Cấu trúc MIME Multipart chuẩn chỉ, an toàn 100% ---
                        ...
                        msg_related.attach(MIMEText("Vui lòng bật chế độ xem HTML để đọc email này.", "plain", "utf-8"))
