...
<style>
    ...
    .vip-badge {
        display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 800;
        color: white; margin-left: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.15);
    ...
    @keyframes shine { 
        ...
        50% { opacity: 1; transform: scale(1.05); box-shadow: 0 0 15px rgba(255,255,255,0.6); } 
    ...
    with tab_send:
            ...
            with bc3:
                if st.button("Tạo hoá đơn", type="primary", use_container_width=True):
                    if amount_input < 10000:
                        ...
                        st.session_state["qr_expire_time"] = time.time() + 600 # 10 phút
                        st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col_data:
            ...
            if df is not None:
                ...
                st.info(f"⚡ **Dự kiến hoàn thành chiến dịch:** {mins} phút {secs} giây")
        ...
        with col_content:
            ...
            email_templates = {
                "📝 Tự soạn mới": "",
                "🎁 Thông báo Khuyến Mãi": f"Kính chào Anh/Chị {{{{name}}}},<br><br>Nhân dịp tháng mới, chúng tôi dành tặng Anh/Chị voucher giảm giá đặc biệt. Vui lòng xem chi tiết bên dưới.",
                "🤝 Thư Cảm Ơn Khách Hàng": f"Chào {{{{name}}}},<br><br>Cảm ơn bạn đã tin tưởng và đồng hành cùng dịch vụ của chúng tôi thời gian qua. Sự ủng hộ của bạn là động lực to lớn của chúng tôi.",
                "💼 Giới thiệu Dịch Vụ Mới": f"Kính gửi {{{{name}}}},<br><br>Chúng tôi vừa ra mắt một giải pháp hoàn toàn mới giúp bạn tiết kiệm 50% chi phí vận hành. Hãy khám phá ngay hôm nay!"
            }
            selected_temp = st.selectbox("📚 Chọn mẫu nội dung có sẵn:", list(email_templates.keys()))
            
            st.markdown("<p style='font-size: 14px; font-weight: 600; color: #b91c1c; margin-bottom: 5px;'>⚠️ LƯU Ý: Vui lòng cuộn chuột trên Web cho ảnh tải hết rồi mới bôi đen Copy và Dán vào đây nhé.</p>", unsafe_allow_html=True)
            
            raw_body = st_quill(value=email_templates[selected_temp], placeholder="Bôi đen Copy/Paste trực tiếp từ Website vào đây...", html=True, key="quill_editor")
        ...
        with col_action2:
            ...
            if st.button("🚀 BẮT ĐẦU CHIẾN DỊCH GỬI MAIL", type="primary", use_container_width=True):
                if df is None: st.error("⚠️ Vui lòng tải lên danh sách Khách hàng!")
                ...
                elif not st.session_state["s_email"] or not st.session_state["s_pwd"]: st.error("⚠️ Lỗi: Bạn chưa điền Email hoặc Mật khẩu ở Bước 1!")
                else:
                    ...
                    for img in soup.find_all("img"):
                        ...
                        elif src.startswith("/") and not src.startswith("//"): src = "https://taynguyenfilm.com" + src
                        elif not src.startswith("http") and not src.startswith("data:"): src = "https://taynguyenfilm.com/" + src

                        if src.startswith("http"):
                            img.attrs = {"src": src, "style": "max-width: 100%; height: auto; display: block; margin: 10px 0;", "alt": "Hình ảnh"}
                        elif src.startswith("data:image"):
                            if img_counter < 2: # CHỈ CHO PHÉP TỐI ĐA 2 ẢNH ĐÍNH KÈM (Base64) TỪ MÁY
                                try:
                                    header, encoded = src.split(",", 1); img_data = base64.b64decode(encoded)
                                    img_counter += 1; ext = "png"
                                    match = re.search(r"image/(.*?);", header)
                                    if match:
                                        ext = match.group(1).lower()
                                        if ext not in ['jpg', 'jpeg', 'png', 'gif']: ext = "png"
                                    cid = f"img_inline_{img_counter}_{int(time.time())}"
                                    inline_images.append({"cid": cid, "data": img_data, "type": ext, "filename": f"image_{img_counter}.{ext}"})
                                    img.attrs = {"src": f"cid:{cid}", "style": "max-width: 100%; height: auto; display: block; margin: 10px 0;"}
                    ...
                    for index, row in df.iterrows():
                        try:
                            ...
                            for img_dict in inline_images:
                                ...
                                img_part.add_header("Content-Disposition", "inline", filename=img_dict["filename"])
