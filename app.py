...
from bs4 import BeautifulSoup # <-- Thêm để tách ảnh khỏi văn bản
from streamlit_quill import st_quill # <-- Thêm ô soạn thảo dán được ảnh
            ...
            if df is None:
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # --- CHỈ SỬA ĐOẠN NÀY: Xử lý nhúng ảnh ---
                        msg = MIMEMultipart("related")
                        msg["From"] = f"{st.session_state['s_name']} <{st.session_state['s_email']}>"
                        msg["To"] = target_email
                        msg["Subject"] = subject
                        
                        # Phân tích nội dung HTML để tìm ảnh được dán vào
                        soup = BeautifulSoup(full_email_content.replace("{{name}}", target_name), "html.parser")
                        img_counter = 0
                        
                        for img in soup.find_all("img"):
                            src = img.get("src", "")
                            # Nếu là ảnh dán trực tiếp (dạng base64)
                            if src.startswith("data:image"):
                                img_counter += 1
                                cid = f"img_inline_{img_counter}"
                                
                                # Tách phần header và data của ảnh
                                header, encoded = src.split(",", 1)
                                img_data = base64.b64decode(encoded)
                                img_type = re.search(r"image/(.*?);", header).group(1)
                                
                                # Tạo phần đính kèm ẩn chứa ảnh
                                img_part = MIMEImage(img_data, _subtype=img_type)
                                img_part.add_header("Content-ID", f"<{cid}>")
                                img_part.add_header("Content-Disposition", "inline")
                                msg.attach(img_part)
                                
                                # Thay thế mã base64 dài ngoằng bằng đường dẫn nội bộ (cid)
                                img["src"] = f"cid:{cid}"
                        
                        # Đính kèm nội dung HTML (đã thay ảnh bằng cid) vào mail
